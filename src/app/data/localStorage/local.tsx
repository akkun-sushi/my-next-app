const saveDataToLocal = <T,>(key: string, data: T | T[]) => {
  try {
    // ローカルストレージに保存する
...
const getDataFromLocal = <T,>(key: string): T | [] => {
  try {
    // ローカルストレージからデータを取得
...
const localStorageSize = (unit: "KB" | "MB" = "KB") => {
  let totalSize = 0;
...
export { saveDataToLocal, getDataFromLocal, localStorageSize };
